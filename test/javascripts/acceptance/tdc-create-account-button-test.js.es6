...
acceptance("TDC Create Account", { site: {
  group_names: [{"name": "solidarity-bloc", "full_name":"Solidarity Bloc"}, {"name":"student-debt", "full_name":"Student Debt Collective"}],
  loggedIn: false }});

only("create account modal submit button works correctly", assert => {
  visit("/");
  ...
  console.log('checked a collective')
  andThen(() => {
...
const solidarityFieldsAllVisible = function() {
  return ["solidarity-how-can-you-help", "solidarity-employment", "solidarity-skills", "phone"].reduce(
    (areVisible, id) => { console.log(id); console.log(exists('#' + id)); return exists('#' + id) && areVisible }
  )
}
